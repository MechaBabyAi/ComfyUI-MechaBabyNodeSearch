...
- 🎯 **智能匹配**：支持节点标题、类型、节点ID、控件名称、控件值、属性名称、属性值的搜索；支持普通、通配符（* ?）、正则三种模式
...
- 📋 **复制控件值**：在节点上右键选择「复制控件值」→ 子菜单列出所有可复制的控件；**选中节点后按 F4** 可直接弹出复制菜单，选择后即复制
...
2. 在设置菜单中，您可以配置：
   ...
   - **复制控件值快捷键**：在「复制控件值」子菜单中可自定义快捷键（默认：F4）
...
### v1.6.3 (2026-02-01)
- 新增正则/通配符搜索：搜索框支持普通、通配符（* ?）、正则三种模式切换
- 搜索结果列表添加滚动条：结果超出一页时右侧显示滚动条
- 新增复制控件值功能：右键菜单「复制控件值」可复制模型名、LoRA 名等；选中节点按 F4 快速弹出复制菜单
- 新增复制快捷键自定义：在 MechaBaby 扩展配置 → 复制控件值 中可修改快捷键（默认 F4）
- 修复快捷键设置对话框中点击「取消」后按键仍被捕获的问题

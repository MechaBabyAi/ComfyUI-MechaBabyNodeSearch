...
- 🔍 **検索ボックスの強化**：ノード検索ダイアログ（Ctrl+F）がドラッグ移動と固定（ピン留め）に対応しました
- 📌 **ウィンドウ固定**：クイックジャンプメニューや検索ダイアログを指定位置に固定して継続的に使用
- 👁️ **自動表示**：ウィンドウ固定後、自動表示を有効にすると、ノードをクリックしたときに自動的にジャンプリストを表示
...
- 🎯 **スマートマッチング**：ノードタイトル、タイプ、ノードID、ウィジェット名、ウィジェット値、プロパティ名、プロパティ値の検索をサポート；通常、ワイルドカード（* ?）、正規表現の3モードに対応
- ✨ **ゴールデンフラッシュハイライト**：ジャンプ後にターゲットノードがゴールド色に点滅し、見つけやすくなります
- 🔄 **Set/Get ノードサポート**：`easy getNode`/`easy setNode`（comfyui-easy-use）および `GetNode`/`SetNode`（ComfyUI-KJNodes）間のジャンプをサポート
- 📋 **ウィジェット値コピー**：ノードを右クリックして「ウィジェット値をコピー」を選択、またはノードを選択して **F4** でコピーメニューを開く
...
2. 設定メニューで以下を設定できます：
   ...
   - **ウィジェットコピーショートカット**：「ウィジェット値をコピー」サブメニューでショートカットをカスタマイズ（デフォルト：F4）
   - **メニュースケール**：クイックジャンプメニューのスケールを調整（50%、75%、100%、125%、150%、またはキャンバスズームに基づく自動調整）

### ウィンドウ固定と自動表示

1. **ウィンドウ固定**：クイックジャンプメニューや検索ダイアログのヘッダーにある 📍 ボタンをクリックして、ウィンドウを固定位置に固定
   - 固定後、ノードにジャンプしてもウィンドウは開いたまま
   - ノードを切り替えたり新しい検索を行ったりしてもウィンドウの位置は固定されたまま
   - 📌 ボタンをクリックして固定を解除

2. **移動可能なダイアログ**：クイックジャンプメニューと検索ダイアログの両方がタイトルバーをドラッグすることで移動可能です。固定時には位置が記憶されます。

2. **自動表示**：ウィンドウが固定されている場合、👁️ ボタンをクリックして自動表示を有効化
   - 有効にすると、任意のノードをクリックすると自動的にジャンプリストが表示されます
   - 再度クリックすると無効化され、手動ショートカットモードに戻ります
   - 自動表示ボタンはウィンドウが固定されている場合のみ表示されます
...
### v1.6.3 (2026-02-01)
- 正規表現・ワイルドカード検索を追加：検索ダイアログで通常、ワイルドカード（* ?）、正規表現の3モード切り替え
- 検索結果リストにスクロールバーを追加：結果が1ページに収まらない場合に右側にスクロールバーを表示
- ウィジェット値コピー機能を追加：右クリックメニューまたは選択ノードでF4押下でモデル名・LoRA名などをコピー
- コピーショートカットのカスタマイズを追加：MechaBaby拡張設定で変更可能（デフォルト：F4）
- ショートカット設定ダイアログで「キャンセル」クリック後にキーリスナーが残る問題を修正

### v1.6.1 (2026-01-31)
- ComfyUI-KJNodes の `GetNode` と `SetNode` へのジャンプサポートを追加
- クイックジャンプパネルのSet/Getノード分類タイトルを統一

### v1.6.0 (2026-01-25)
- メイン検索ダイアログ（Ctrl+F）に移動可能および固定機能を追加
- 検索ダイアログを最適化：固定時に位置を記憶し、ジャンプ後も開いたままに
- Toast通知を強化：90%の透明度とフォントの視認性を向上させた、洗練されたブルー半透明スタイル
- UIの一貫性を向上：すべてのダイアログがタイトルバーによるドラッグ移動に対応
- クイックジャンプ（F1）ロジックを改善：GET/SETの矢印方向とEasy Useノードの分類表示を修正

### v1.5.0 (2026-01-08)
- ウィンドウ固定機能を追加：クイックジャンプメニューを指定位置に固定
- 自動表示機能を追加：ウィンドウ固定後、自動表示を有効にすると、ノードをクリックしたときに自動的にジャンプリストを表示
- ユーザー体験の改善：ブロッキング型のalertダイアログを非ブロッキング型のtoast通知に置き換え
- GET/SETノードのクイックジャンプメニューと右クリックメニューでの矢印方向を修正
- Easy Useノードの表示を修正：GETノードは左パネルにSETノードを表示、SETノードは右パネルにGETノードを表示
- 「接続なし」表示を修正：Easy Useノードが存在する場合に「接続なし」を表示しないように修正
- toast通知を強化：フォントサイズを大きくし、視認性を向上
